...
class UnifiedUploader:
    ...
    def __init__(self, platform, username, password, host, repo_id, proxy=None, path=None, file_path=None, max_file_size_mb=999999):
        """
        ...
        :param proxy: Proxy dict (opcional)
        :param file_path: Ruta al archivo a subir (opcional en init, requerido en upload)
        :param max_file_size_mb: Límite máximo de tamaño de archivo en MB
        """
    ...
    def _validate_file(self) -> None:
        ...
        if file_size_mb > self.max_file_size_mb:
            raise ValueError(f"Archivo excede límite de {self.max_file_size_mb}MB")
    ...
    def _safe_error_message(self, error: Exception, context: str) -> str:
        ...
        error_messages = {
            "ValueError": "Ocurrió un problema con los datos proporcionados. Por favor, verifica e intenta de nuevo.",
            "FileNotFoundError": "El archivo especificado no se encuentra. Asegúrate de que la ruta es correcta.",
            "requests.RequestException": "No se pudo conectar al servidor. Verifica tu conexión o intenta más tarde.",
        ...
        default_message = "Ocurrió un error inesperado. Por favor, intenta de nuevo o contacta al soporte."
    ...
    def get_moodle_token(self):
        """
        Obtiene automáticamente el token de Moodle usando las credenciales
    ...
    def login(self):
        ...
        try:
            if self.platform == "Moodle":
                ...
                # Intentar obtener token después del login
                ...
                return True  # Asumimos éxito; fallará en upload si credenciales incorrectas
    ...
    def _upload_with_token(self, progressfunc=None, args=(), tokenize=False):
        """
        Sube un archivo a Moodle usando el método de token y genera una URL pública
        Adaptado a síncrono desde el cliente async proporcionado.
        ...
        try:
            ...
            # Para progreso, usar MultipartEncoderMonitor si progressfunc está presente
            if progressfunc:
                b = uuid.uuid4().hex
            ...
            # Crear evento para obtener URL pública (adaptado de async)
            ...
            data = {'url': public_url, 'normalurl': public_url, 'name': os.path.basename(self.file_path)}
            return None, data
    ...
    def upload_file(self, file_path=None, progressfunc=None, args=(), tokenize=False):
        ...
        if self.platform == "Moodle":
            # Primero intentar con token si está disponible
            if self.token:
    ...
    def _upload_to_moodle(self, progressfunc, args, tokenize):
        try:
            ...
            upload_data = {
                ...
                'areamaxbytes': (None, str(1024 * 1024 * 1024 * 4)),  # Hack para bypass de límite (4GB)
            ...
            data['name'] = os.path.basename(self.file_path)
    ...
    def _upload_to_next(self):
        try:
            ...
            if self.path:
                destino_temporal = f"{self.host}/remote.php/dav/files/{self.username}{self.path}/{nombre_final}"
            else:
            ...
            return None, {'url': enlace_descarga, 'name': nombre_final}
        except Exception as e:
    ...
    def logout(self):
        try:
            if self.platform == "Moodle":
                ...
                # No logout específico para WebDAV/Next, solo cerrar sesión
                self.session.close()
        except Exception as e:
            pass
